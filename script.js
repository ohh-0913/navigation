...
function setupDailyQuote() {
    ...
    function checkAndUpdateQuote() {
        ...
        if (savedQuote) {
            ...
            // 一言存在，但更新时间超过1小时或者是页面刷新，获取新的一言
            if ((currentTime - quoteData.timestamp > 3600000) || 
                performance.navigation.type === 1 || // 使用performance API检测页面刷新
                !sessionStorage.getItem('quoteDisplayed')) { // 使用sessionStorage检测新会话
                
                fetchNewQuote();
                sessionStorage.setItem('quoteDisplayed', 'true');
            } else {
                // 使用保存的一言
